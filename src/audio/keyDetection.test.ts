...
const rotateProfile = (profile: number[], root: number) => Array.from({ length: 12 }, (_, pitchClass) => profile[(pitchClass - root + 12) % 12])
...
describe('musical key detection', () => {
  ...
  it('detects an A minor chroma profile', () => {
    expect(detectKeyFromChroma(rotateProfile(MINOR_PROFILE, 9))).toMatchObject({
      root: 9,
      mode: 'minor',
      key: 'A minor',
      shortKey: 'Am',
