...
test('renders the full WebDJ studio layout', async ({ page }) => {
  ...
  await expect(page.getByRole('region', { name: 'Performance quick controls' })).toBeVisible()
  await expect(page.getByRole('region', { name: 'Audio output settings' })).toBeVisible()
})

test('keeps mixer controls accessible after the visual redesign', async ({ page }) => {
  await page.goto('/')
