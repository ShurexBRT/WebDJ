...
test('renders the full WebDJ studio layout', async ({ page }) => {
  ...
  await expect(page.getByRole('region', { name: 'Sampler panel', exact: true })).toBeVisible()
...
test('keeps mixer and deck controls accessible after the visual redesign', async ({ page }) => {
  ...
  await expect(page.getByRole('textbox', { name: 'Search loaded tracks', exact: true })).toBeVisible()
